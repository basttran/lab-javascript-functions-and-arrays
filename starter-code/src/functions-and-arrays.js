...
function maxOfTwoNumbers(firstNumber, secondNumber) {
  var sign = Math.sign(firstNumber - secondNumber);
  if (sign >= 0) {
...
//  console.log("sumArray(numberArray) / numberArray.length");
...
// console.log(averageNumbers(wordLengthArray));
...
function greatestProduct(matrix) {
  ...
  for (i = 0; i < matrix.length - 1; i++) {
    submatrix[i]=[];
    for (j = 0; j < matrix[0].length - 1; i++) {
      submatrix[i][j] = matrix[i][j]*matrix[i+1][j]*matrix[i][j+1]*matrix[i+1][j+1];
